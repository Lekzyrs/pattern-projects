...
export class HauntedManor {
    private weaponEquipmentFacade: WeaponEquipmentFacade;

    constructor() {
        const characterClasses = Object.values(CharacterClass) as CharacterClass[]; // Получаем массив всех значений CharacterClass
        const randomIndex = Math.floor(Math.random() * characterClasses.length); // Генерируем случайный индекс
        const randomClass = characterClasses[randomIndex]; // Выбираем случайный класс
